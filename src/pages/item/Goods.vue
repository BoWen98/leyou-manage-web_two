    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon v-if="props.item.saleable" @click="offSale(props.item.id)">下架</v-btn>
          <v-btn icon v-else @click="onSale(props.item.id)">上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
    ...
    methods: {
      ...
      async editGoods(oldGoods) {
        if (oldGoods.saleable) {
          ...
          this.$message.confirm("请先下架商品,再做修改!").then(() => {
            this.offSale(oldGoods.id)
          });
          return;
        }
        // 发起请求，查询商品详情和skus
        oldGoods.spuDetail = await this.$http.loadData("/item/spu/detail/" + oldGoods.id);
        oldGoods.skus = await this.$http.loadData("/item/sku/list/" + oldGoods.id);
        // 修改标记
        this.isEdit = true;
        // 控制弹窗可见：
        this.show = true;
      ...
      deleteGoods(spu) {
        if (spu.saleable) {
          //如果是上架商品
          this.$message.confirm("请先下架商品,再做删除!").then(() => {
            this.offSale(spu.id)
          });
          return;
        }
        //发起请求,删除商品
        this.$message.confirm("确认删除吗?").then(() => {
          this.$http.delete("/item/goods/" + spu.id)
            .then(() => {
      ...
      offSale(spuId) {
        //发起请求,下架商品
        this.$message.confirm("确认下架吗?").then(() => {
          this.$http.put("/item/goods/off/" + spuId)
            .then(() => {
      ...
      onSale(spuId) {
        //发起请求,上架商品
        this.$message.confirm("确认上架吗?").then(() => {
          this.$http.put("/item/goods/on/" + spuId)
            .then(() => {
